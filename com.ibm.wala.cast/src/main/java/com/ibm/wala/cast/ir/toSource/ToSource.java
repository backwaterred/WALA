...
public abstract class ToSource {
  ...
  protected static class CodeGenerationContext implements CAstVisitor.Context {
    private final CAstSourcePositionRecorder positionMap;
    ...
    public CodeGenerationContext(
        TypeInference types, IntegerUnionFind mergePhis, boolean isTopLevel, CAstSourcePositionRecorder positionMap) {
      this.types = types;
    ...
    public CodeGenerationContext(CodeGenerationContext parent, int precedence) {
      ...
      this.positionMap = parent.positionMap;
    }
  ...
  protected class RegionTreeNode {
    ...
    private void indent(StringBuffer text, int level) {
    ...
    public CAstEntity toCAstEntity() {
      CAstNode root = toCAst();
    ...
    protected class ToCAst {
      ...
      protected class Visitor implements AstPreInstructionVisitor {
        ...
        public Visitor(
          ...
          this.positionMap = (CAstSourcePositionRecorder) c.getSourceMap();
          root.visit(this);
          if (root.hasDef()) {
        ...
        // PRE: node set to new node, ir.getMethod returns instance of AstMethod
        // EFFECTS: Modifies positionMap
        private void markPosition(int iIndex) {
          ...
          positionMap.setPosition(node, pos);
        }
        ...
        public void visitArrayLoad(SSAArrayLoadInstruction instruction) {
          ...
          markPosition(instruction.iIndex());
        }
        ...
        // It might be the next block, it might not
        private ISSABasicBlock getLoopSuccessor(ISSABasicBlock controlBB) {
          ...
          for (ISSABasicBlock nextBB : cfg.getNormalSuccessors(controlBB)) {
            if (DFS.getReachableNodes(cfg, Collections.singleton(nextBB)).contains(controlBB)) {
              assert loopBB == null;
              loopBB = nextBB;
            }
          }
        ...
        public void visitConditionalBranch(SSAConditionalBranchInstruction instruction) {
          ...
          if (loop != null && loop.getLoopControl().equals(branchBB)) {
            ...
            if (LoopType.DOWHILE.equals(loopType)) {
              ...
              Set<ISSABasicBlock> elseBlock = HashSetFactory.make(loop.getLoopExits());
        ...
        @Override
        public void visitLoadMetadata(SSALoadMetadataInstruction inst) {
          node = ast.makeConstant(inst.getToken());
          markPosition(inst.iIndex());
        }
        ...
        @Override
        public <T> void visitUnspecified(SSAUnspecifiedInstruction<T> inst) {
          node = ast.makeNode(CAstNode.PRIMITIVE, ast.makeConstant(inst.getPayload()));
          markPosition(inst.iIndex());
        }

        @Override
        public <T> void visitUnspecifiedExpr(SSAUnspecifiedExprInstruction<T> inst) {
          node = ast.makeNode(CAstNode.PRIMITIVE, ast.makeConstant(inst.getPayload()));
          markPosition(inst.iIndex());
        }
      }
  ...
  public void toJava(
    ...
    CAstEntity entity = root.toCAstEntity();
    CAstNode ast = entity.getAST();
    CAstSourcePositionRecorder positionMap = (CAstSourcePositionRecorder) entity.getSourceMap();
    ...
    try (StringWriter sw = new StringWriter()) {
      ...
      toJava.visit(ast, new CodeGenerationContext(types, root.mergePhis, true, positionMap), toJava);
      if (codeRecorder != null) {
              ...
              s -> {
                ...
                try {
                  if (s.containsKey("v")) {
                    ...
                    ev.visit(v, new CodeGenerationContext(types, root.mergePhis, true, positionMap), ev);
                    o.print(" = ");
                  }
                  ev.visit(e, new CodeGenerationContext(types, root.mergePhis, true, positionMap), ev);
                  o.print("\n");
                  o.flush();
                } catch (Throwable e1) {
                  assert false : e1;
